...
comp <- bind_cols(zoo, ind_clust) %>% 
  select(clust_zoo, Actinopterygii:Trichodesmium) %>% # zoo starts at "Actinopterygii" instead of "Acantharea" because no Acantharea present in bathy layer
  gather(Actinopterygii:Trichodesmium, key = "taxon", value = "conc") %>% 
...
message("We should aim for ~ 15 modalities in other partitionings.")
...
png(file = "plots/analysis/bathy/10.plankton_dendrogram_max_model.png", width = 9.79, height = 7.96, units = "in", res = 300)
plot(clust_zoo, main = "Cluster dendrogram on plankton data in bathypelagic layer for max model")
# Choose number of clusters
nclust <- 15
# Plot clusters
rect.hclust(clust_zoo, k=nclust)
...
eig_env %>% 
  ...
  xlab("Principal componant") +
  ylab("Explained variance") +
  ggtitle("PC and explained variance of env data PCA in bathy layer") +
  theme_classic() +
  # y scale as a percentage and remove white space
  scale_y_continuous(labels=scales::percent, expand = c(0,0))
ggsave(file = "plots/analysis/bathy/10.env_pca_variance_explained.png")

# Biplot of env PCA
png(file = "plots/analysis/bathy/10.env_pca_biplot_1_2.png", width = 9.79, height = 7.96, units = "in", res = 300)
...
title(
  ...
  main = "PCA biplot of env data in bathy layer"
) 
dev.off()
...
# Plot dendrogram
png(file = "plots/analysis/bathy/10.env_dendrogram.png", width = 9.79, height = 7.96, units = "in", res = 300)
plot(clust_env, main = "Cluster dendrogram env data bathypelagic layer")
# Choose number of clusters
nclust <- 12
# Plot clusters
...
ggsave(g, file = "plots/analysis/bathy/10.partitioning_modalities.png")
...
zoo_anosim <- data_anosim %>% select(Actinopterygii:Trichodesmium)
...
anosim_res %>% 
  ...
  ggtitle("Part of plankton data variance explained by partitionings in bathy layer")
ggsave(file = "plots/analysis/bathy/10.partitioning_anosim.png")
...
anosim_res %>% 
  ...
  save(file="data/10.bathy_anosim.Rdata")
...
groups %>% 
  mutate(layer = study_layer) %>% 
  save(file="data/10.bathy_partitionings.Rdata")
