...
projects <- read_csv("data/extract_zoo/UVP5_projects_selected.csv")
...
# write it to a file
write_tsv(selected_samples_info, file.path(data_dir, "UVP5_samples_selected.tsv"))


## Compare with the original selection by Thelma ----

# read the samples used by Thelma
thelma_samples <- read_csv(file.path(data_dir, "list_profiles_ecopart-thelma.csv"), col_types=cols()) %>%
...
missing_in_current %>% group_by(title) %>% summarise(samples=str_c(profile, collapse=",")) %>% write_tsv(file.path(data_dir, "to_finish_sorting.tsv"))
