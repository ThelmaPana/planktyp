...
message(nrow(subset), " profiles in mesoinf layer")
...
# Plot dendrogram
png(file = "plots/analysis/mesoinf/09.plankton_dendrogram.png", width = 9.79, height = 7.96, units = "in", res = 300)
plot(clust_zoo, main = "Cluster dendrogram on plankton data in mesoinf layer")
...
ind_clust %>% 
  ...
  ggtitle("Number of profiles per plankton cluster in mesoinf layer") +
  theme_classic() +
  # y scale as a percentage and remove white space
  scale_y_continuous(expand = c(0,0))
ggsave(file = "plots/analysis/mesoinf/09.plankton_clusters_profile_nb.png")
...
eig %>% 
  ...
  ggtitle("PC and explained variance of plankton PCA in mesoinf layer") +
  theme_classic() +
  # y scale as a percentage and remove white space
  scale_y_continuous(labels=scales::percent, expand = c(0,0))
ggsave(file = "plots/analysis/mesoinf/09.plankton_pca_variance_explained.png")
...
ggsave(file = "plots/analysis/mesoinf/09.plankton_pca_biplot_1_2.png")
...
ggplot() +
  ...
  # Bigger points in the legend
  guides(colour = guide_legend(override.aes = list(size=3)))
ggsave(file = "plots/analysis/mesoinf/09.plankton_pca_biplot_2_3.png")
...
ind_plot %>% 
  ...
  # Bigger points in the legend
  guides(colour = guide_legend(override.aes = list(size=3))) +
  ggtitle("Map of mesoinf plankton clusters")
ggsave(file = "plots/analysis/mesoinf/09.plankton_clusters_map.png")
...
comp %>% 
  ...
  ggtitle("Composition of mesoinf plankton clusters")
ggsave(file = "plots/analysis/mesoinf/09.plankton_clusters_composition.png")
...
png(file = "plots/analysis/mesoinf/09.plankton_dendrogram_max_model.png", width = 9.79, height = 7.96, units = "in", res = 300)
plot(clust_zoo, main = "Cluster dendrogram on plankton data in mesoinf layer for max model")
# Choose number of clusters
nclust <- 15
# Plot clusters
rect.hclust(clust_zoo, k=nclust)
...
eig_env %>% 
  ...
  xlab("Principal componant") +
  ylab("Explained variance") +
  ggtitle("PC and explained variance of env data PCA in mesoinf layer") +
  theme_classic() +
  # y scale as a percentage and remove white space
  scale_y_continuous(labels=scales::percent, expand = c(0,0))
ggsave(file = "plots/analysis/mesoinf/09.env_pca_variance_explained.png")

# Biplot of env PCA
png(file = "plots/analysis/mesoinf/09.env_pca_biplot_1_2.png", width = 9.79, height = 7.96, units = "in", res = 300)
...
# Plot dendrogram
png(file = "plots/analysis/mesoinf/09.env_dendrogram.png", width = 9.79, height = 7.96, units = "in", res = 300)
plot(clust_env, main = "Cluster dendrogram env data mesoinf layer")
# Choose number of clusters
nclust <- 12
# Plot clusters
...
ggsave(g, file = "plots/analysis/mesoinf/09.partitioning_modalities.png")
...
anosim_res %>% 
  ...
  ggtitle("Part of plankton data variance explained by partitionings in mesoinf layer")
ggsave(file = "plots/analysis/mesoinf/09.partitioning_anosim.png")
...
anosim_res %>% 
  ...
  save(file="data/09.mesoinf_anosim.Rdata")

# Plankton composition of plankton clusters
comp %>% 
  mutate(layer = study_layer) %>% 
  save(file="data/09.mesoinf_plankton_cluster_comp.Rdata")

# Partitionings for anosim
groups %>% 
  mutate(layer = study_layer) %>% 
  save(file="data/09.mesoinf_partitionings.Rdata")
